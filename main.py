...
def admin_panel(request: Request):
    ...
    function updateSelector() {{
        ...
        if (slugs.length > 0) {{ selector.value = slugs[0]; loadPageData(); }}
    }}
    ...
    function loadPageData() {{
        ...
        if (dlLinks.length === 0 && p.download_link) {{ dlLinks = [p.download_link]; }}
        ...
        if (pubs.length === 0) {{ addPubRow(); }} else {{ pubs.forEach(pub => addPubRow(pub.type || 'video', pub.url || '', pub.desc || '')); }}
        document.getElementById('plans-container').innerHTML = '';
        let plans = p.plans || [];
        if (plans.length === 0) {{ addPlanRow(); }} else {{ plans.forEach(plan => addPlanRow(plan.name || '', plan.price || '', plan.features || '', plan.link || '', plan.highlight || false)); }}
    ...
    function createPage() {{
        ...
        allPages[slug] = {{ page_name: name, chatbot_id: 'gzEjAzK1VCE72hJ_hBfA4', hero_title: name, hero_subtitle: '', hero_text: '', affiliate_link: '', affiliate_text: '', publications: [], plans: [], social_links: {{ facebook: '', whatsapp: '', youtube: '', tiktok: '', telegram: '', instagram: '' }}, bank_transfer_info: {{}}, download_links: [], download_instructions: '' }};
        saveData(true);
    }}
    ...
    function deletePage() {{
        ...
        if(confirm('¿Seguro que quieres eliminar esta página?')) {{ delete allPages[slug]; saveData(true); }}
    }}
    ...
    function addPubRow(type = 'video', url = '', desc = '') {{
        ...
        div.innerHTML = `<select class="pub-type w-full bg-slate-800 rounded p-2 mb-2 border border-slate-700 outline-none"><option value="video" ${{type=='video'?'selected':''}}>Video de YouTube</option><option value="image" ${{type=='image'?'selected':''}}>Imagen</option></select><input type="text" class="pub-url w-full bg-slate-800 rounded p-2 mb-2 border border-slate-700 outline-none" placeholder="URL (Embed YouTube o Imagen)" value="${{url}}"><textarea class="pub-desc w-full bg-slate-800 rounded p-2 border border-slate-700 outline-none" placeholder="Descripción">${{desc}}</textarea><button onclick="this.parentElement.remove()" class="mt-2 text-red-500 text-xs hover:text-red-400"><i class="fas fa-trash mr-1"></i>Eliminar</button>`;
        c.appendChild(div);
    }}
    ...
    function addPlanRow(name = '', price = '', features = '', link = '', highlight = false) {{
        ...
        const div = document.createElement('div');
        div.className = 'bg-slate-900 p-4 rounded border border-slate-700';
        div.innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 gap-2"><input type="text" class="p-name bg-slate-800 rounded p-2 border border-slate-700 outline-none focus:border-cyan-500" placeholder="Nombre Plan (Ej: Oro)" value="${{name}}"><input type="text" class="p-price bg-slate-800 rounded p-2 border border-slate-700 outline-none focus:border-cyan-500" placeholder="Precio (Ej: $199)" value="${{price}}"></div><textarea class="p-features w-full bg-slate-800 rounded p-2 my-2 border border-slate-700 outline-none focus:border-cyan-500" placeholder="Características">${{features}}</textarea><input type="text" class="p-link w-full bg-slate-800 rounded p-2 mb-2 border border-slate-700 outline-none focus:border-cyan-500" placeholder="ID de Plan de PayPal (Ej: P-78W24779DJ167620XNKNUHYY)" value="${{link}}"><div class="flex justify-between items-center mt-2"><label class="text-sm flex items-center gap-2 cursor-pointer"><input type="checkbox" class="p-highlight accent-cyan-500" ${{highlight ? 'checked' : ''}}> Resaltar (Más Popular)</label><button onclick="this.parentElement.parentElement.remove()" class="text-red-500 text-xs hover:text-red-400"><i class="fas fa-trash mr-1"></i>Eliminar</button></div>`;
        c.appendChild(div);
    }}
    ...
    async function saveData(reloadSelector = false) {{
        ...
        if (!slug) {{ showToast('❌ No hay página seleccionada.'); return; }}
        let pubsArray = [];
        document.querySelectorAll('#pubs-container > div').forEach(div => {{ const urlInput = div.querySelector('.pub-url'); if(urlInput && urlInput.value.trim()) {{ pubsArray.push({{ type: div.querySelector('.pub-type').value, url: urlInput.value, desc: div.querySelector('.pub-desc').value }}); }} }});
        let plansArray = [];
        document.querySelectorAll('#plans-container > div').forEach(div => {{ const nameInput = div.querySelector('.p-name'); if(nameInput && nameInput.value.trim()) {{ plansArray.push({{ name: nameInput.value, price: div.querySelector('.p-price').value, features: div.querySelector('.p-features').value, link: div.querySelector('.p-link').value, highlight: div.querySelector('.p-highlight').checked }}); }} }});
        let dlLinksArray = [];
        document.querySelectorAll('#dl-links-container > div').forEach(div => {{ const urlInput = div.querySelector('.dl-url'); if(urlInput && urlInput.value.trim()) {{ dlLinksArray.push(urlInput.value.trim()); }} }});
        allPages[slug] = {{
            ...
            publications: pubsArray, plans: plansArray,
            ...
            download_links: dlLinksArray, download_instructions: document.getElementById('download_instructions').value
        }};
        try {{
        ...
        }} catch(e) {{ showToast('❌ Error de conexión.'); console.error(e); }}
    }}
    ...
    async function loadStats() {{
        try {{
            ...
            let html = '';
            countryKeys.sort((a,b) => countries[b] - countries[a]).forEach(c => {{ html += `<div class="flex justify-between items-center bg-slate-900 p-2 rounded"><span class="text-sm text-slate-300">${{c}}</span><span class="text-cyan-400 font-bold">${{countries[c]}}</span></div>`; }});
            ...
            let leadsHtml = '';
            if(leads.length === 0) {{ leadsHtml = '<p class="text-slate-500 text-sm">Aún no se han capturado correos.</p>'; }}
            else {{
                leads.slice().reverse().forEach(l => {{
                    let stageText = l.follow_up_stage === 0 ? 'Email Inicial Enviado' : l.follow_up_stage === 1 ? 'Seguimiento 1 Enviado' : l.follow_up_stage === 2 ? 'Seguimiento 2 Enviado' : 'Embudo Finalizado';
                    leadsHtml += `<div class="bg-slate-900 p-3 rounded border border-slate-700"><div class="flex justify-between"><span class="text-cyan-400 font-bold text-sm">${{l.name}} - $${{l.email}}</span><span class="text-slate-500 text-xs">${{l.date.split('T')[0]}}</span></div><div class="text-slate-400 text-xs mt-1">Interés: $${{l.interaction}}</div><div class="text-emerald-400 text-xs mt-1">🤖 IA: $${{stageText}}</div></div>`;
                }});
            }}
    ...
    async function loadAIConfig() {{
        try {{
            ...
            const data = await res.json();
            const defaults = {{ stage1_days: 2, stage1_subject: "🚀 {{name}}, descubre el poder de la IA", stage1_body: "Hola {{name}},...", stage2_days: 5, stage2_subject: "🔥 {{name}}, esto es lo que estás dejando atrás...", stage2_body: "Hola {{name}},...", stage3_days: 10, stage3_subject: "⏳ {{name}}, tu acceso VIP está por expirar", stage3_body: "Hola {{name}},..." }};
    ...
    async function saveAIConfig() {{
        const payload = {{ stage1_days: parseInt(document.getElementById('s1_days').value), stage1_subject: document.getElementById('s1_subject').value, stage1_body: document.getElementById('s1_body').value, stage2_days: parseInt(document.getElementById('s2_days').value), stage2_subject: document.getElementById('s2_subject').value, stage2_body: document.getElementById('s2_body').value, stage3_days: parseInt(document.getElementById('s3_days').value), stage3_subject: document.getElementById('s3_subject').value, stage3_body: document.getElementById('s3_body').value }};
    ...
    async function saveUpdateConfig() {{
        const payload = {{ latest_version: document.getElementById('upd_version').value, download_url: document.getElementById('upd_url').value, force_update: document.getElementById('upd_force').checked, update_message: document.getElementById('upd_message').value }};
    ...
    async function createManualLicense() {{
        ...
        if(result.status === 'success') {{
            msgDiv.innerHTML = `✅ Licencia creada: <br><br> <input type="text" value="${{result.key}}" readonly class="w-full bg-slate-950 text-cyan-400 p-2 rounded mt-2 cursor-pointer select-all" onclick="this.select()">`;
            document.getElementById('lic_key').value = result.key;
        }} else {{ msgDiv.innerText = "❌ " + (result.message || "Error al crear la licencia."); }}
    ...
    async function changePassword() {{
        ...
        msgDiv.classList.remove('hidden');
        showToast(result.message);
        if(result.status === 'success') {{ document.getElementById('current_pwd').value = ''; document.getElementById('new_pwd').value = ''; document.getElementById('confirm_pwd').value = ''; }}
    }}
    ...
    function setMktType(type) {{
        ...
        ['seo','tiktok','reddit'].forEach(t => {{
            ...
            if(t === type) {{ btn.classList.remove('border-slate-700'); btn.classList.add('border-purple-500', 'bg-purple-900/30'); }}
            else {{ btn.classList.add('border-slate-700'); btn.classList.remove('border-purple-500', 'bg-purple-900/30'); }}
        }});
    }}
    ...
    async function generateMktContent() {{
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Gemini está redactando...';
        resultBox.classList.remove('hidden');
        resultArea.value = '⏳ Procesando...';
        try {{
            ...
            if(!contentType.includes('application/json')) {{
                ...
                resultArea.value = '❌ El servidor devolvió HTML (no JSON). Código HTTP: ' + res.status + '\\n\\nEsto significa que el endpoint /api/generate_marketing NO está desplegado en Render.\\nAsegúrate de pegar el código completo del endpoint en tu archivo Python principal y hacer deploy.';
                showToast('❌ Endpoint no encontrado.');
                return;
            }}
            const data = await res.json();
            if(res.ok && data.status === 'success') {{
                ...
                resultArea.value = '❌ ' + (data.message || data.detail || 'Error desconocido.');
                showToast('❌ ' + (data.message || 'Error.'));
            }}
        }} catch(e) {{
            console.error('Error completo:', e);
            resultArea.value = '❌ Error de conexión: ' + e.message + '\\n\\nPosibles causas:\\n1. El servidor está dormido (Render free tier). Visita primero la web principal.\\n2. El endpoint no está desplegado.\\n3. Problema de red.\\n\\nAbre la consola (F12) para más detalles.';
...
def api_save_pages(request: Request, data: dict):
    ...
    try:
        if save_all_pages({"pages": data}): return {"message": "✅ Página guardada correctamente."}
        else: return {"message": "❌ Error al guardar en Supabase (Revisa los logs de Render)."}
    except Exception as e:
    ...
    for p in c.get('plans', []):
        if p.get('name'):
            ...
            if plan_link.startswith("P-"):
                ...
                paypal_scripts += f"""<script>if (typeof paypal !== 'undefined') {{ paypal.Buttons({{ style: {{ shape: 'rect', color: 'blue', layout: 'vertical', label: 'subscribe' }}, createSubscription: function(data, actions) {{ return actions.subscription.create({{ plan_id: '{plan_link}' }}); }}, onApprove: function(data, actions) {{ alert('¡Suscripción procesada con éxito!'); }} }}).render('#paypal-btn-{plan_link}'); }}</script>"""
            else:
            ...
            plans_html += f"""<div class="relative bg-slate-800 p-8 rounded-xl border {highlight_classes} transition-all duration-300 hover:-translate-y-2 hover:shadow-xl flex flex-col">{badge}<h3 class="text-xl font-bold text-white mb-2">{p.get('name', '')}</h3><div class="text-4xl font-extrabold text-cyan-400 mb-4">{p.get('price', '')}<span class="text-base font-normal text-slate-500">/mes</span></div><p class="text-slate-300 text-sm mb-6 flex-grow">{features_html}</p>{pay_btn_html}<div class="mt-2">{bank_btn_html}</div></div>"""

    social = c.get('social_links') if isinstance(c.get('social_links'), dict) else {}
    social_html = ""
    for k, icon in [('facebook','fa-facebook-f'),('whatsapp','fa-whatsapp'),('youtube','fa-youtube'),('tiktok','fa-tiktok'),('telegram','fa-telegram-plane'),('instagram','fa-instagram')]:
        if social.get(k): social_html += f'<a href="{social.get(k)}" target="_blank" rel="noopener noreferrer" class="bg-slate-800 hover:bg-cyan-500 hover:text-slate-900 text-slate-300 p-3 rounded-full transition-all duration-300 transform hover:-translate-y-1"><i class="fab {icon}"></i></a>'
...
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>BLENIN.G.77 - Institutional Trading AI</title>
    ...
    <meta name="keywords" content="trading IA, bot metatrader 5, inteligencia artificial trading, EA forex, bot automático MT5, blenin77">
    <meta name="robots" content="index, follow">
    <meta property="og:title" content="BLENIN.G.77 - IA Predictiva para Trading Institucional">
    <meta property="og:description" content="El mejor sistema de IA para automatizar tus operaciones en MT5.">
...
<body class="text-slate-300">
    ...
    <section id="features" class="py-20 container mx-auto px-6"><h2 class="text-3xl font-bold text-center text-white mb-12">Tecnología de Nivel Institucional</h2><div class="grid md:grid-cols-3 gap-8"><div class="bg-slate-900 p-8 rounded-xl border border-slate-800 hover:border-cyan-500 transition group"><div class="text-cyan-400 text-3xl mb-4"><i class="fas fa-fish"></i></div><h3 class="text-xl font-bold text-white mb-2">Enjambre 3D</h3><p class="text-slate-400 text-sm">500 agentes virtuales simulan el futuro del mercado en milisegundos.</p></div><div class="bg-slate-900 p-8 rounded-xl border border-slate-800 hover:border-cyan-500 transition group"><div class="text-cyan-400 text-3xl mb-4"><i class="fas fa-shield-alt"></i></div><h3 class="text-xl font-bold text-white mb-2">Agente Centinela</h3><p class="text-slate-400 text-sm">Lee Reuters, CNBC y la Fed en tiempo real. Bloquea al bot ante crashes.</p></div><div class="bg-slate-900 p-8 rounded-xl border border-slate-800 hover:border-cyan-500 transition group"><div class="text-cyan-400 text-3xl mb-4"><i class="fas fa-brain"></i></div><h3 class="text-xl font-bold text-white mb-2">Cerebro Global</h3><p class="text-slate-400 text-sm">Red neuronal descentralizada que aprende de todos los usuarios.</p></div></div></section>
    ...
    {SOCIAL_SECTION_HTML}
    <footer class="bg-slate-950 py-10 border-t border-slate-800"><div class="container mx-auto px-6 text-center"><p class="text-slate-500 text-sm mb-4 max-w-3xl mx-auto"><strong>Aviso de Riesgo:</strong> El trading de divisas y CFDs implica un riesgo sustancial y no es adecuado para todos los inversores. El rendimiento pasado no es indicativo de resultados futuros.</p><p class="text-slate-600 text-xs">&copy; 2024 BLENIN.G.77 THE BEST FUTURE FOR YOU. Creado por Lenin Benitez.</p></div></footer>
    {BANK_MODAL_HTML}
    <div id="download-box" class="max-w-md mx-auto bg-slate-800 p-8 rounded-xl border border-cyan-500 shadow-lg text-center mt-6" style="display: {DISPLAY_STYLE};"><i class="fas fa-check-circle text-emerald-400 text-4xl mb-4"></i><h4 class="text-xl font-bold text-cyan-400 mb-4">¡Listo! Aquí tienes tu descarga:</h4><div class="text-slate-300 text-sm mb-6 text-left bg-slate-900 p-4 rounded-lg border border-slate-700">{DOWNLOAD_INSTRUCTIONS_HTML}</div><div class="flex flex-col gap-3">{DOWNLOAD_BUTTONS_HTML}</div></div>
...
def blog_index():
    return HTMLResponse("""<html><head><title>Blog - BLENIN77</title><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700;800&display=swap" rel="stylesheet"><style>body { font-family: 'Inter', sans-serif; background-color: #020617; }</style></head><body class="text-slate-300 p-10"><div class="max-w-3xl mx-auto"><h1 class="text-3xl font-bold text-cyan-400 mb-6">📈 Blog de Trading Inteligente</h1><ul class="mt-6 space-y-4"><li class="bg-slate-800 p-4 rounded"><a href="/blog/como-invertir-con-ia-forex" class="text-xl hover:text-cyan-400">¿Cómo invertir en Forex usando IA?</a></li><li class="bg-slate-800 p-4 rounded"><a href="/blog/mejores-estrategias-automaticas-mt5" class="text-xl hover:text-cyan-400">Top 5 Estrategias para MT5</a></li></ul><a href="/" class="text-cyan-400 mt-6 inline-block">← Volver al Inicio</a></div></body></html>""")
...
def blog_article(slug: str):
    ...
    return HTMLResponse(f"""<html><head><title>{title} - BLENIN77</title><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700;800&display=swap" rel="stylesheet"><style>body {{ font-family: 'Inter', sans-serif; background-color: #020617; }}</style></head><body class="text-slate-300 p-10"><div class="max-w-3xl mx-auto prose prose-invert"><a href="/blog" class="text-cyan-400 mb-4 inline-block">← Volver al Blog</a>{article_html}<div class="mt-8 p-6 bg-slate-800 rounded-xl border border-cyan-500 text-center"><h3 class="text-xl font-bold text-white mb-2">¿Listo para automatizar?</h3><a href="/#pricing" class="bg-cyan-500 text-slate-900 font-bold py-2 px-6 rounded hover:bg-cyan-400">Ver Planes</a></div></div></body></html>""")
        ...
        if info.get("email", "").lower() == email:
            if info["active"]:
                ...
                if email not in [l.get("email") for l in stats_db["captured_leads"]]:
                    stats_db["captured_leads"].append({"name": email.split("@")[0], "email": email, "interaction": "Intento recuperar licencia expirada", "date": datetime.now().isoformat(), "follow_up_stage": 0, "last_email_sent": datetime.now().isoformat()})
                    save_dbs(licenses_db, trials_db, stats_db, admin_password_db, ai_agent_config, bot_update_config)
...
def generate_marketing_content(request: Request, data: MarketingRequest):
    try:
        if not verify_admin(request):
            return {"status": "error", "message": "❌ No autorizado. Inicia sesión de nuevo."}
        if not GEMINI_API_KEY:
            return {"status": "error", "message": "❌ La variable GEMINI_API_KEY no está configurada en Render. Ve a Environment y añádela."}
        topic = (data.topic or "").strip() or "automatizar trading con IA en MT5"
        base_url = "https://blenin77-server.onrender.com/#pricing"
        if data.content_type == "seo":
            prompt = f"""Eres un experto en SEO y copywriting. Escribe un artículo de blog de 600-800 palabras sobre: "{topic}".
Requisitos:
- Título H1 (# Título) atractivo con la palabra clave.
- 3-5 subtítulos H2 (## Subtítulo).
- Menciona el bot BLENIN.G.77 destacando: IA Predictiva, Enjambre de 500 agentes, Modo Híbrido MT5 + Noticias macroeconómicas.
- Usa **negritas** en conceptos clave.
- Termina con CTA hacia: {base_url}
- Formato Markdown.
Responde SOLO con el artículo."""
        elif data.content_type == "tiktok":
            prompt = f"""Eres un guionista viral de TikTok experto en trading. Crea un guion de 45-60 segundos sobre: "{topic}".
Formato estricto:
[HOOK 0-3s]: Frase impactante.
[DESARROLLO 3-45s]: 3 puntos clave.
[CTA 45-60s]: Llamada a BLENIN.G.77 ({base_url}).
Incluye sugerencias de texto en pantalla y música.
Responde SOLO con el guion."""
        else:
            prompt = f"""Eres un usuario real de Reddit (r/Forex). Escribe un post genuino sobre: "{topic}".
- Tono casual, como trader compartiendo experiencia.
- Menciona BLENIN.G.77 de forma natural.
- No uses lenguaje comercial.
- Título (máx 100 caracteres).
- Cuerpo de 200-300 palabras.
- Enlace sutil a {base_url}
Responde SOLO con el post."""
        content = generate_dynamic_content_with_llama(prompt, max_tokens=2000)
        if not content:
            return {"status": "error", "message": "Gemini no respondió. Revisa los logs de Render. Posibles causas: API Key incorrecta, modelo incorrecto, o cuota agotada."}
        return {"status": "success", "content": content, "type": data.content_type}
    except Exception as e:
        print(f"🚨 EXCEPCIÓN en /api/generate_marketing: {e}", flush=True)
        return {"status": "error", "message": f"Error interno del servidor: {str(e)}"}

@app.get("/api/debug_marketing")
def debug_marketing():
    return {
        "gemini_key_set": bool(GEMINI_API_KEY),
        "gemini_model": GEMINI_MODEL,
        "endpoint_exists": True,
        "message": "Si ves endpoint_exists: true, el endpoint está desplegado correctamente."
    }

# ==========================================
# 🛡️ ALERTA DE RIESGO DEL BOT
# ==========================================
...
def hotmart_webhook(request: Request):
    try:
        ...
        if data.get("event") == "PURCHASE_APPROVED" or data.get("event_type") == "PURCHASE_APPROVED":
            ...
            email = buyer_data.get("email") or data.get("data", {}).get("purchase", {}).get("buyer", {}).get("email")
            ...
            save_dbs(licenses_db, trials_db, stats_db, admin_password_db, ai_agent_config, bot_update_config)
            send_email(email, "✅ Pago Confirmado - Aquí tienes tu Licencia BLENIN77", f"¡Gracias por tu compra!\n\nTu pago ha sido confirmado.\n\nTu clave de licencia:\n{key}\n\nPlan: {plan_upper}\nDuración: 30 días\n\nPara descargar el sistema: https://blenin77-server.onrender.com/\n\nEquipo BLENIN77.")
            send_email(SMTP_EMAIL, f"💰 ¡Nueva Venta en Hotmart! Plan {plan_upper}", f"Cliente: {email}\nPlan: {plan_upper}\nLicencia: {key}")
...
def make_payment_webhook(data: MakeWebhookData):
    ...
    try:
        ...
        key = generate_license_key(plan_upper)
        licenses_db[key] = {"hwid": None, "expires": (datetime.now() + timedelta(days=data.duration_days)).isoformat(), "active": True, "plan": plan_upper, "email": data.email.lower()}
        save_dbs(licenses_db, trials_db, stats_db, admin_password_db, ai_agent_config, bot_update_config)
        send_email(data.email, "✅ Pago Confirmado - Aquí tienes tu Licencia BLENIN77", f"¡Gracias por tu compra!\n\nTu clave de licencia:\n{key}\n\nPlan: {plan_upper}\nDuración: {data.duration_days} días\n\nPara descargar: https://blenin77-server.onrender.com/\n\nEquipo BLENIN77.")
        send_email(SMTP_EMAIL, f"💰 ¡Nueva Venta Automática! Plan {plan_upper}", f"Cliente: {data.email}\nPlan: {plan_upper}\nLicencia: {key}")
